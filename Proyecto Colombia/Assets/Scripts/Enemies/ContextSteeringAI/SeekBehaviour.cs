...
public class SeekBehaviour : SteeringBehaviour
{
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, ContextSteeringAIData aiData)
    {
        ...
        {
            if (aiData._targets == null || aiData._targets.Count <= 0)
            {
            ...
            else
            {
                ...
                aiData._currentTarget = aiData._targets.OrderBy(target => Vector2.Distance(target.position, transform.position)).FirstOrDefault();
            }
        }

        //cache the last position only if we still see the target (if the targets collection is not empty)
        if (aiData._currentTarget != null && aiData._targets != null && aiData._targets.Contains(aiData._currentTarget))
            targetPositionCached = aiData._currentTarget.position;
